...
from micro_so import Micro, AsyncMicro, DefaultAioHttpClient
from micro_so._utils import is_dict
...
logging.getLogger("micro_so").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Micro]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Micro(base_url=base_url, api_key=api_key, team_id=team_id, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncMicro]:
    ...
    async with AsyncMicro(
        base_url=base_url, api_key=api_key, team_id=team_id, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
