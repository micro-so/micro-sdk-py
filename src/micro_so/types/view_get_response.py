...
class ViewBundleWithRecordsRecords(BaseModel):
...
class ViewBundleWithRecordsView(BaseModel):
    """
    A view (saved configuration for displaying records of a given object type) plus its select/filter/sort children. Properties in select/filter/sort are referenced by slug.
    """
...
class ViewBundleWithRecords(BaseModel):
    """Returned by `GET /views/{viewId}?include=records`.

    Same `records` shape as the standalone list-view-records endpoint.
    """
...
ViewGetResponse: TypeAlias = Union[ViewBundle, ViewBundleWithRecords]
