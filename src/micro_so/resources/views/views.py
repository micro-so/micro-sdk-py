...
class ViewsResource(SyncAPIResource):
    ...
    def create(
        ...
        *,
        ...
        return self._post(
            path_template(
                "/v2/prism/{path_team_id}/{object_type}/views", path_team_id=path_team_id, object_type=object_type
            ),
    ...
    def update(
        ...
        *,
        ...
        return self._patch(
            path_template(
                "/v2/prism/{path_team_id}/{object_type}/views/{view_id}",
    ...
    def list(
        ...
        *,
        ...
        if not object_type:
            raise ValueError(f"Expected a non-empty value for `object_type` but received {object_type!r}")
        return self._get(
            path_template("/v2/prism/{team_id}/{object_type}/views", team_id=team_id, object_type=object_type),
            options=make_request_options(
    ...
    def delete(
        ...
        *,
        ...
        return self._delete(
            path_template(
                "/v2/prism/{team_id}/{object_type}/views/{view_id}",
                team_id=team_id,
                object_type=object_type,
                view_id=view_id,
            ),
    ...
    def get(
        ...
        *,
        ...
        """Returns the view bundle.

        Pass `?include=records` to also fetch a page of records
        selected by the view in the same call; the response is then wrapped as
        `{view, records}`.

        Args:
          cursor: Forwarded to the records sub-resource when `include=records`.

          include: Comma-separated list of optional sub-resources to inline. Currently the only
              recognized value is `records` — when present, the response is `{view, records}`
              rather than the bare view bundle.

          limit: Forwarded to the records sub-resource when `include=records`.

          page: Forwarded to the records sub-resource when `include=records`.
        ...
        return cast(
            ...
            self._get(
                ...
                cast_to=cast(Any, ViewGetResponse),  # Union types cannot be passed in as arguments in the type system
            ),
        )
