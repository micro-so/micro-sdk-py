...
class GrantResource(SyncAPIResource):
    ...
    def update(
        ...
        *,
        ...
        return self._put(
            path_template(
                "/v2/prism/{path_team_id}/event/{event_id}/grant", path_team_id=path_team_id, event_id=event_id
            ),
    ...
    def get(
        ...
        *,
        ...
        if not event_id:
            raise ValueError(f"Expected a non-empty value for `event_id` but received {event_id!r}")
        return self._get(
            path_template("/v2/prism/{team_id}/event/{event_id}/grant", team_id=team_id, event_id=event_id),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
